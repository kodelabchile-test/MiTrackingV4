...
2. (Opcional) Si quieres montar otra ubicación para el frontend, exporta la ruta absoluta mediante `FRONTEND_SRC`. De lo contrario la pila montará automáticamente `frontend/` del repositorio.

   ```bash
   export FRONTEND_SRC="/ruta/a/tu/frontend"
   ```

   En PowerShell: `setx FRONTEND_SRC "C:\\ruta\\a\\frontend"` (abre una nueva sesión para que el cambio tome efecto).
...
3. Lanza la pila de desarrollo desde la raíz del repositorio:
   ...
   * Si Compose no puede resolver la ruta relativa por defecto (por ejemplo, al invocar el comando desde fuera del repositorio), define `FRONTEND_SRC` con la ubicación correcta del frontend.
